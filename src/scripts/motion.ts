...
// The original design always plays the intro; the swing-rig is the only
// element suppressed by CSS when reduced-motion is on (see global.css).
// Checking matchMedia here caused blank hero pages on Windows + Brave
// because Brave inherits the system "Reduce animations" setting.
...
if (heroFigure) { heroFigure.style.cursor = 'pointer'; heroFigure.addEventListener('click', replayIntro); }
if (spiderHero) { spiderHero.style.cursor = 'pointer'; spiderHero.addEventListener('click', replayIntro); }
